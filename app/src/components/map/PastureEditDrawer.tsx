...
interface PastureEditDrawerProps {
  pasture: Pasture
  open: boolean
  onClose: () => void
}
...
export function PastureEditDrawer({ pasture, open, onClose }: PastureEditDrawerProps) {
  ...
  const [lastPastureId, setLastPastureId] = useState(pasture.id)

  // Reset form when pasture changes
  if (pasture.id !== lastPastureId) {
    setLastPastureId(pasture.id)
