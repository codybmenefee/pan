...
- Paddocks should provide reasonable water access
- Pastures without water access require different management
- Water failure is an operational emergency, not a planning consideration

This constraint influences paddock placement. When water sources are defined, the AI considers proximity when generating paddock geometry.`,
    },
    {
      heading: 'Modeling Water Access',
      content: `Water sources are modeled as zones within pastures:`,
      codeExample: {
        language: 'typescript',
        filename: 'schema.ts',
...
The platform currently treats all water zones as equivalent—present or absent. Future versions may incorporate reliability and capacity.`,
    ...
    {
      heading: 'Impact on Paddock Recommendations',
      content: `When water zones are defined, the AI considers them during paddock generation:

**Zone inclusion preference:**
Paddocks that include or are adjacent to water sources are preferred over paddocks far from water.

**Access path consideration:**
If water is outside the recommended paddock, the AI may note this in the justification.
...
- Pasture rotation may be constrained by water, not forage
...
**Step 4: Associate with pasture**
- Link the zone to its parent pasture
- If water serves multiple pastures, create zones in each

Water zones appear on the map within their pasture boundaries.`,
    },
    {
      heading: 'When Water Constraints Override Forage',
      content: `In extreme cases, water availability trumps forage optimization:

**No water in ready pasture:**
Even if NDVI is excellent and rest days are sufficient, a pasture without functioning water shouldn't receive animals. The farmer should reject such recommendations.

**Water in recovering pasture:**
If the only available water is in a pasture still recovering, the farmer faces a trade-off. The platform doesn't make this decision—it surfaces the information.
