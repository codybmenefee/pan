...
export const pastures: ArticleContent = {
  title: 'Pastures',
  description:
    'How the platform represents and tracks pastures. Key attributes, status calculation logic, threshold overrides, and managing pasture boundaries.',
  sections: [
    {
      heading: 'What a Pasture Represents',
      content: `A pasture is the fundamental management unit in the platform:

**Physical definition:**
A discrete area where livestock can be contained—typically bounded by fencing, natural barriers, or infrastructure.

**Management definition:**
The smallest unit you'd consider for a grazing decision. You move animals "to a pasture" not "to a corner of land."
...
Pastures can vary in size from a few hectares to hundreds. The platform treats them equivalently—paddock sizing scales to pasture area.`,
    },
    {
      heading: 'Key Attributes',
      content: `Each pasture record contains:`,
      codeExample: {
        language: 'typescript',
        filename: 'schema.ts',
  ...
  updatedAt: v.string(),
    ...
    },
    {
      ...
      content: `Pasture status is computed from NDVI and rest days:`,
      codeExample: {
        language: 'typescript',
...
// else: status remains 'recovering' (Orange)`,
    ...
    },
    {
      ...
      content: `**ready (Green)**
NDVI meets or exceeds threshold AND rest days meet or exceed minimum. This pasture is a candidate for grazing.

**almost_ready (Yellow)**
NDVI meets threshold but rest days are between 14-21. Approaching readiness—don't graze yet unless necessary.

**recovering (Orange)**
Either NDVI below threshold OR rest days below 14. Needs more time. The platform will avoid recommending this pasture unless no alternatives exist.

**grazed (Gray)**
Less than 7 days since last grazing event. Too recent to consider regardless of NDVI readings.

Status colors appear on the map and in pasture lists, providing at-a-glance understanding of farm condition.`,
    },
    {
      heading: 'Per-Pasture Threshold Overrides',
      content: `Default thresholds apply farm-wide, but individual pastures can override:

**When to use overrides:**

*Irrigated pastures* - May have higher NDVI baseline and faster recovery. Override: lower rest period requirement.
...
**Setting overrides:**
Overrides are stored per-pasture and applied when computing status:`,
      codeExample: {
        language: 'typescript',
...
// When computing status, check for pasture-level override
...
Coordinates are [longitude, latitude] pairs (GeoJSON standard). The outer ring defines the pasture boundary; inner rings can define exclusions (though rarely used).
...
- Simpler boundaries (fewer vertices) process faster`,
    },
    {
      ...
      content: `Pasture boundaries can be created through:
