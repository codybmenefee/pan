...
- At least one paddock has geometry defined
...
**Farm Map** - Paddock boundaries with color-coded status:
...
**Status Summary** - Total paddocks, current grazing location, and overall farm health metrics.

The map provides spatial context. The status indicators show which paddocks are candidates for grazing.`,
    },
    {
...
The AI analyzes:
- Current NDVI readings across all paddocks
- Rest days since last grazing event for each paddock
- Previous grazing sections in the target paddock
...
**Recommended Section** - A specific area within the target paddock, shown as a polygon on the map. Sections are approximately 20% of paddock area by default.
...
**Reasoning** - 2-3 bullet points explaining why this paddock and section were selected. Typical factors include:
- NDVI values relative to threshold
- Rest days since last grazing
- Position relative to previously grazed sections

**Section Justification** - Detailed text explaining the section placement, what areas to target, and any warnings about conditions.`,
    },
    {
...
**Modify** - Make adjustments to the section geometry or change the target paddock before approval.
...
2. A grazing event is logged for the target paddock
3. Tomorrow's brief will account for today's grazing location
4. Rest day calculations update accordingly

The platform accumulates history over time. More usage produces better recommendations as the system learns your paddock rotation patterns and farm-specific characteristics.
